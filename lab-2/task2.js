  ...
  validator: {
    $jsonSchema: {
      ...
      properties: {
        ...
        max_places:   { bsonType: "int",     minimum:1, description: "int>=1 required" },
        companyId:    { bsonType: "objectId", description: "fkey to Company1._id" }
      }
    }
  }
});
...
// a) Company1
over let companyIds = db.Company1.insertMany([
...
db.Trip1.aggregate([
  ...
  { $project: {
      ...
      availablePlaces:    { $subtract: ["$max_places", { $sum: "$reservations.no_tickets" }] }
  }}
]);
...
db.createCollection("Trip3", {
  validator: {
    $jsonSchema: {
      bsonType: "object",
      required: ["name", "destination", "date", "max_places", "companyId", "ratings"],
      properties: {
        _id:             { bsonType: "objectId" },
        name:            { bsonType: "string" },
        destination:     { bsonType: "string" },
        date:            { bsonType: "date" },
        max_places:      { bsonType: "int", minimum: 1 },
        companyId:       { bsonType: "objectId", description: "fkey to Company3._id" },
    ...
    $jsonSchema: {
      ...
      properties: {
        ...
        reservations: {
          ...
          items: {
            bsonType: "object",
            required: ["tripId", "reservationId", "no_tickets", "name", "destination", "date", "companyId", "companyName"],
            properties: {
              name:        { bsonType: "string" },
              destination: { bsonType: "string" },
              date:        { bsonType: "date" },
              ...
              tripId:      { bsonType: "objectId", description: "fkey to Trip3._id" },
              reservationId:      { bsonType: "objectId", description: "fkey to Reservation3._id" },
              companyId:   { bsonType: "objectId", description: "fkey to Company3._id" }
            }
          }
        }
...
const trips = [
  { name: "Mazury Tour", destination: "Mazury", date: ISODate("2025-03-10"), max_places:20, companyId: comp1Id },
  { name: "Tatry Hike", destination: "Tatry", date: ISODate("2025-07-15"), max_places:15, companyId: comp1Id },
  { name: "City Break", destination: "Wrocław", date: ISODate("2025-05-20"), max_places:25, companyId: comp2Id }
];
...
db.Company3.updateOne({ _id: comp1Id }, { $set: {
  trips: [
    { tripId: trip1Id, name: "Mazury Tour", destination: "Mazury", date: ISODate("2025-03-10"), max_places:20 },
    { tripId: trip2Id, name: "Tatry Hike", destination: "Tatry", date: ISODate("2025-07-15"), max_places:15 }
  ]
}});
db.Company3.updateOne({ _id: comp2Id }, { $set: {
  trips: [
    { tripId: trip3Id, name: "City Break", destination: "Wrocław", date: ISODate("2025-05-20"), max_places:25 }
  ]
}});
...
const res1 = db.Reservation3.insertOne({ personId:p1._id, tripId:trip1Id, no_tickets:2 });
const res2 = db.Reservation3.insertOne({ personId:p2._id, tripId:trip1Id, no_tickets:1 });
...
[res1, res2].forEach(({ insertedId, ops }, idx) => {
  ...
  db.Person3.updateOne({ _id: person._id }, { $push: {
    reservations: {
      ...
      companyName: (r.companyId.equals(comp1Id) ? "TravelCo" : "AdventureTime")
    }
  }});
});

// f) Wstawiamy ratings dla Trip3
db.Trip3.updateOne({ _id: trip1Id }, { $push: { ratings: [
  { personId: p1._id, firstname: p1.firstname, lastname: p1.lastname, rating: 5 },
  { personId: p2._id, firstname: p2.firstname, lastname: p2.lastname, rating: 4 }
]}});
...
const newRes = db.Reservation1.insertOne({ personId: person1Id, tripId: trip1Id, no_tickets: 2 });

// 3.3. Dodanie nowej rezerwacji (Model3):
// a) do kolekcji rezerwacji
const trip3Id = db.Trip3.findOne()._id;
const res3 = db.Reservation3.insertOne({ personId: person1, tripId: trip3Id, no_tickets: 2 });
...
// lub w Company3:
db.Company3.find({ "trips.destination":"Mazury", "trips.date": { $gte: ISODate("2025-01-01") }});

// 1.7. Aktualizacja firmy (Model1)
db.Company1.updateOne({ _id: comp1Id },{ $set:{ address:"ul. Nowa 15, Warszawa"}});
// 3.7. Aktualizacja firmy (Model3)
db.Company3.updateOne({ _id: comp1Id },{ $set:{ address:"ul. Nowa 15, Warszawa"}});

// 1.8. Wyszukiwanie osób, które zarezerwowały konkretne tripId (Model1)
db.Person1.aggregate([
  { $lookup:{ from:"Reservation1", localField:"_id", foreignField:"personId", as:"reservations" }},
  { $match:{ "reservations.tripId": ObjectId("<WSTAW_TRIP1_ID>") }},
  { $project:{ _id:0, firstname:1, lastname:1 }}
]);

// 3.8. Wyszukiwanie osób (Model3)
db.Person3.find(
  { "reservations.tripId": ObjectId("<WSTAW_TRIP1_ID>") },
